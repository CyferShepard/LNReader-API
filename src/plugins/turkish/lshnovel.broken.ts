...
class LSHNovel implements Plugin.PluginBase {
  ...
  async popularNovels(
    ...
    if (filters.type.value.length) link += filters.type.value.map((i) => `&lang[]=${i}`).join("");
    ...
    const body = await fetchApi(link, { headers }).then((result) => result.text());
  ...
  async parseNovelAndChapters(novelUrl: string): Promise<Plugin.SourceNovel> {
    ...
    novel.cover = loadedCheerio("img.wp-post-image").attr("data-src") || loadedCheerio("img.wp-post-image").attr("src");
    ...
    novel.summary = loadedCheerio('div[itemprop="description"]').find("br").replaceWith("\n").end().text();
    ...
    loadedCheerio(".eplister")
      .find("li")
      .each(function () {
        const chapterName = loadedCheerio(this).find(".epl-num").text() + " - " + loadedCheerio(this).find(".epl-title").text();
