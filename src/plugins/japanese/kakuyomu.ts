import { fetchText } from "@libs/fetch.ts";
...
class KakuyomuPlugin implements Plugin.PluginBase {
  ...
  async popularNovels(
    ...
    const url = new URL(`/rankings/${filters.genre.value}/${filters.period.value}`, this.site);
    if (pageNo > 1) {
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    const json = JSON.parse($('script#__NEXT_DATA__[type="application/json"]').html() || "{}");
    ...
    const joinChapters: {
      ...
      const chapter = tableOfContentsChapter.find((c) => c.episodeUnions.some((e) => e.__ref === `Episode:${v.id}`))?.chapter;
      return {
        chapter: chapters.find((c) => c.id === chapter?.__ref.replace("Chapter:", "")),
        episode: v,
      };
    });
    ...
    const novel: Plugin.SourceNovel = {
      ...
      status: work.serialStatus === "COMPLETED" ? NovelStatus.Completed : NovelStatus.Ongoing,
      summary: work.introduction,
      chapters: joinChapters.map((v) => {
        return {
          name: v.chapter?.title ? `${v.chapter?.title} - ${v.episode?.title}` : v.episode?.title ?? "",
  ...
  async searchNovels(searchTerm: string, pageNo: number): Promise<Plugin.NovelItem[]> {
    ...
    const json = JSON.parse($('script#__NEXT_DATA__[type="application/json"]').html() || "{}");
    const works = Object.values(json?.props?.pageProps?.__APOLLO_STATE__ ?? {}).filter((v) => {
