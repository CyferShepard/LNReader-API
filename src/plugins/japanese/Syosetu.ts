...
// const novelStatus = require('@libs/novelStatus.ts');
// const isUrlAbsolute = require('@libs/isAbsoluteUrl.ts');
// const parseDate = require('@libs/parseDate.ts');
...
class Syosetu implements Plugin.PluginBase {
  ...
  async popularNovels(
    ...
    const getNovelsFromPage = async (pagenumber: number) => {
      ...
      if (!filters.genre.value) {
        ...
        url += `rank/${filters.genre.value.length === 1 ? "isekailist" : "genrelist"}/type/${filters.ranking.value}_${
          filters.genre.value
        }${filters.modifier.value === "total" ? "" : `_${filters.modifier.value}`}/?p=${pagenumber}`;
      }
      ...
      if (parseInt(loadedCheerio(".is-current").html() || "1") !== pagenumber) return [];
  ...
  private async parseChaptersFromPage(loadedCheerio: cheerio.CheerioAPI): Promise<Plugin.ChapterItem[]> {
    ...
    loadedCheerio(".p-eplist__sublist").each((_, element) => {
      ...
      const releaseDate = loadedCheerio(element).find(".p-eplist__update").text().trim().split(" ")[0].replace(/\//g, "-");
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    const novel: Plugin.SourceNovel = {
      ...
      cover: defaultCover,
      chapters: [],
      genres: loadedCheerio('meta[property="og:description"]').attr("content")?.split(" ").join(","), // Get genres from meta tag
    };
    ...
    if (!lastPageLink) {
      ...
      const pagePromises = Array.from({ length: totalPages }, (_, i) =>
        fetchApi(`${this.novelPrefix}${novelPath}?p=${i + 1}`).then((r) => r.text())
      );
      ...
      pageResults.forEach((pageBody) => {
        ...
        pageCheerio(".p-eplist__sublist").each((_, element) => {
          ...
          const chapterUrl = chapterLink.attr("href");
          const chapterName = chapterLink.text().trim();
          const releaseDate = pageCheerio(element).find(".p-eplist__update").text().trim().split(" ")[0].replace(/\//g, "-");
