...
class Smakolykytl implements Plugin.PluginBase {
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    const novel: Plugin.SourceNovel = {
      ...
      status: book?.project?.status_translate.includes("Триває") ? NovelStatus.Ongoing : NovelStatus.Completed,
    };
  ...
  async searchNovels(searchTerm: string): Promise<Plugin.NovelItem[]> {
    ...
    json?.projects
      ?.filter(({ title, id }) => title.toLowerCase().includes(searchTitle) || String(id) == searchTerm)
      ?.forEach((novel) =>
        novels.push({
          name: novel.title,
          cover: novel.image.url,
