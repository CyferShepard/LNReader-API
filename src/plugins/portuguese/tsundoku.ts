...
class TsundokuPlugin implements Plugin.PluginBase {
  ...
  parseDate(date: string): string {
    ...
    return dayjs(`${year}-${monthMapping[month.normalize("NFD").replace(/[\u0300-\u036f]/g, "")]}-${day}`).toISOString();
  }
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    ...
    const title = chapterTitle.replace(novelTitle, "").replace(/^\W+/, "").trim();

    const spoilerContent = loadedCheerio("#readerarea .collapseomatic_content").html();
    if (spoilerContent) {
      return `<h1>${title}</h1>\n${spoilerContent}`;
    }
