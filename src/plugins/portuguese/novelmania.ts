...
class NovelMania implements Plugin.PluginBase {
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    const novel: Plugin.SourceNovel = {
      path: novelPath,
      name:
        loadedCheerio("div.col-md-8 > div.novel-info > div.d-flex.flex-row.align-items-center > h1").text().trim() ||
        "Sem título",
    };
    ...
    novel.name =
      loadedCheerio("div.col-md-8 > div.novel-info > div.d-flex.flex-row.align-items-center > h1").text().trim() || "Sem título";
    novel.summary = loadedCheerio("div.tab-pane.fade.show.active > div.text > p").text().trim() || "";
    novel.cover = loadedCheerio("div.novel-img > img.img-responsive").attr("src") || defaultCover;
    ...
    const status = loadedCheerio("div.novel-info > span.authors.mb-3").text().trim();
    switch (status) {
      case "Ativo":
    ...
    loadedCheerio("div.accordion.capitulo > div.card > div.collapse > div.card-body.p-0 > ol > li").each((i, el) => {
      const chapterName = `${loadedCheerio(el).find("a > span.sub-vol").text().trim()} - ${loadedCheerio(el)
        .find("a > strong")
        .text()
        .trim()}`;
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    const response = await fetchApi(`${this.site}${chapterPath}`).then((res) => res.text());
  ...
  resolveUrl = (path: string, isNovel?: boolean) => this.site + (isNovel ? "/book/" : "/chapter/") + path;
