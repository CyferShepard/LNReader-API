...
class TuNovelaLigera implements Plugin.PagePlugin {
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel & { totalPages: number }> {
    ...
    novel.cover = novelCover.attr("data-src") || novelCover.attr("src") || novelCover.attr("data-cfsrc") || defaultCover;

    loadedCheerio(".post-content_item").each(function () {
      const detailName = loadedCheerio(this).find(".summary-heading > h5").text().trim();
      ...
      switch (detailName) {
        ...
        case "Autor(es)":
          ...
          break;
        case "Estado":
          novel.status = detail.includes("OnGoing") || detail.includes("Updating") ? NovelStatus.Ongoing : NovelStatus.Completed;
          break;
      }
    });
