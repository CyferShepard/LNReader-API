...
class NovelBin implements Plugin.PluginBase {
  ...
  parseNovels(loadedCheerio: CheerioAPI) {
    ...
    loadedCheerio(".col-novel-main .list-novel .row").each((i, el) => {
      ...
      let novelCover = loadedCheerio(el).find("img.cover").attr("data-src")?.replace("_200_89", "");

      if (!novelCover) {
        novelCover = loadedCheerio(el).find("img.cover").attr("src")?.replace("_200_89", "");
      }
      const novelUrl = loadedCheerio(el).find("h3.novel-title > a").attr("href");
