...
class NovelUpdates implements Plugin.PluginBase {
  ...
  parseNovels(loadedCheerio: CheerioAPI) {
    ...
    loadedCheerio("div.search_main_box_nu").each((idx, ele) => {
      ...
      const novelUrl = loadedCheerio(ele).find(".search_title > a").attr("href");
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    novel.status = loadedCheerio("#editstatus").text().includes("Ongoing") ? "Ongoing" : "Completed";
    ...
    loadedCheerio("li.sp_li_chp").each((i, el) => {
      ...
      const chapterUrl = "https:" + loadedCheerio(el).find("a").first().next().attr("href");
  ...
  async getChapterBody(loadedCheerio: CheerioAPI, domain: string[], url: string) {
    let bloatElements = [];
    let chapterTitle = "";
    let chapterContent = "";
    let chapterText = "";
    ...
    switch (targetDomain) {
      case "anotivereads":
        chapterTitle = loadedCheerio("#comic-nav-name").first().text() || "Title not found";
      ...
      case "asuratls":
        ...
        break;
      case "daoist":
        chapterTitle = loadedCheerio(".chapter__title").first().text() || "Title not found";
        chapterContent = loadedCheerio(".chapter__content").html()!;
        chapterText = `<h2>${chapterTitle}</h2><hr><br>${chapterContent}`;
        break;
      case "fictionread":
        bloatElements = [".content > style", ".highlight-ad-container", ".meaning", ".word"];
        bloatElements.forEach((tag) => loadedCheerio(tag).remove());
        chapterTitle = loadedCheerio(".title-image span").first().text() || "Title not found";
      ...
      case "genesistudio":
        ...
        break;
      case "helscans":
        chapterTitle = loadedCheerio(".entry-title-main").first().text() || "Title not found";
        const chapterString_helscans = "Chapter " + chapterTitle.split("Chapter")[1].trim();
      ...
      case "hiraethtranslation":
        const bloatAttributes = ["data-lazy-srcset", "data-lazy-src", "data-lazy-sizes", "data-ll-status"];
        ...
        chapterTitle = loadedCheerio("li.active").first().text() || "Title not found";
        chapterContent = loadedCheerio(".text-left").html()!;
        chapterText = `<h2>${chapterTitle}</h2><hr><br>${chapterContent}`;
        break;
      case "hostednovel":
        chapterTitle = loadedCheerio("#chapter-title").first().text() || "Title not found";
      ...
      case "infinitenoveltranslations":
         ...
         */
        const link_infinite = loadedCheerio(".cm-entry-summary > p > a").attr("href")!;
        if (link_infinite) {
        ...
        chapterTitle = loadedCheerio(".cm-entry-title").text() || "Title not found";
        chapterText = `<h2>${chapterTitle}</h2><hr><br>${chapterContent}`;
        break;
      ...
      case "isotls": // mii translates
        bloatElements = ["footer", "header", "nav", ".ezoic-ad", ".ezoic-adpicker-ad", ".ezoic-videopicker-video"];
        bloatElements.forEach((tag) => loadedCheerio(tag).remove());
        chapterTitle = loadedCheerio("head title").first().text() || "Title not found";
        chapterContent = loadedCheerio("main article").html()!;
        chapterText = `<h2>${chapterTitle}</h2><hr><br>${chapterContent}`;
        break;
      case "ko-fi":
        const matchResult_kofi = loadedCheerio('script:contains("shadowDom.innerHTML")')
          .html()
      ...
      case "mirilu":
        ...
        bloatElements.forEach((tag) => loadedCheerio(tag).remove());
        const titleElement_mirilu = loadedCheerio(".entry-content p strong").first();
      ...
      case "novelplex":
        ...
        bloatElements.forEach((tag) => loadedCheerio(tag).remove());
        chapterTitle = loadedCheerio(".halChap--jud").first().text() || "Title not found";
      ...
      case "novelworldtranslations":
        ...
        loadedCheerio(".entry-content a")
          .filter((_, el) => {
            return loadedCheerio(el).attr("href")?.includes("https://novelworldtranslations.blogspot.com") || false;
          })
          .each((_, el) => {
            loadedCheerio(el).parent().remove();
          });
        chapterTitle = loadedCheerio(".entry-title").first().text() || "Title not found";
      ...
      case "rainofsnow":
        ...
        bloatElements = [".responsivevoice-button", ".zoomdesc-cont p img", ".zoomdesc-cont p noscript"];
      ...
      case "sacredtexttranslations":
        bloatElements = [".entry-content blockquote", ".entry-content div", ".reaction-buttons"];
        bloatElements.forEach((tag) => loadedCheerio(tag).remove());
        chapterTitle = loadedCheerio(".entry-title").first().text() || "Title not found";
        chapterContent = loadedCheerio(".entry-content").html()!;
        chapterText = `<h2>${chapterTitle}</h2><hr><br>${chapterContent}`;
      ...
      case "skydemonorder":
         ...
         */
        const ageVerification_skydemon = loadedCheerio("main").text().toLowerCase()!;
        ...
        chapterTitle = `${loadedCheerio(".pl-4 h1").first().text() || "Title not found"} | ${
          loadedCheerio(".pl-4 div").first().text() || "Title not found"
        }`;
        chapterContent = loadedCheerio("#startContainer + * > *").first().html()!;
        if (!chapterContent) {
          chapterContent = `${loadedCheerio(
            "#chapter-body"
          ).html()!}<hr><br>There could be missing content, please check in webview.`;
        }
        chapterText = `<h2>${chapterTitle}</h2><hr><br>${chapterContent}`;
        break;
      case "stabbingwithasyringe":
        ...
        bloatElements = [".has-inline-color", ".wp-block-buttons", ".wpcnt", "#jp-post-flair"];
        bloatElements.forEach((tag) => loadedCheerio(tag).remove());
        chapterContent = loadedCheerio(".entry-content").html()!;
      ...
      case "tinytranslation":
        bloatElements = [".content noscript", ".google_translate_element", ".navigate", ".post-views", "br"];
        bloatElements.forEach((tag) => loadedCheerio(tag).remove());
        chapterTitle = loadedCheerio(".title-content").first().text() || "Title not found";
      ...
      case "wattpad":
        ...
        break;
      case "webnovel":
        chapterTitle = loadedCheerio(".cha-tit .pr .dib").first().text() || "Title not found";
      ...
      case "wetriedtls":
        const scriptContent_wetried =
          loadedCheerio('script:contains("p dir=")').html() || loadedCheerio('script:contains("u003c")').html();
      ...
      case "wuxiaworld":
        ...
        bloatElements.forEach((tag) => loadedCheerio(tag).remove());
        chapterTitle = loadedCheerio("h4 span").first().text() || "Title not found";
        chapterContent = loadedCheerio(".chapter-content").html()!;
        chapterText = `<h2>${chapterTitle}</h2><hr><br>${chapterContent}`;
        break;
      case "yoru":
      ...
      case "zetrotranslation":
        ...
        if (titleElement_zetro.length) {
          ...
          chapterTitle = loadedCheerio(".active").first().text() || "Title not found";
          chapterText = `<h2>${chapterTitle}</h2><hr><br>${chapterContent}`;
        }
        break;
    }
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    ...
    if (!result.ok) {
       ...
       */
      throw new Error(`Could not reach site (${result.status}), try to open in webview.`);
    }
    ...
    let isBlogspot = blogspotSources.some(
      (source) => source && blogspotKeywords.some((keyword) => source.toLowerCase().includes(keyword))
    );
    ...
    let isWordPress = wordpressSources.some(
      (source) => source && wordpressKeywords.some((keyword) => source.toLowerCase().includes(keyword))
    );
    ...
    if (!isWordPress && !isBlogspot) {
      ...
      bloatElements = [".button-container", ".ChapterNav", ".ch-bottom", ".separator"];
      bloatElements.forEach((tag) => loadedCheerio(tag).remove());
      chapterTitle =
        loadedCheerio(".entry-title").first().text() || loadedCheerio(".post-title").first().text() || "Title not found";
      chapterContent =
        loadedCheerio(".content-post").html() || loadedCheerio(".entry-content").html() || loadedCheerio(".post-body").html()!;
      if (chapterTitle && chapterContent) {
        chapterText = `<h2>${chapterTitle}</h2><hr><br>${chapterContent}`;
      }
    } else if (isWordPress) {
      ...
      const chapterSubtitle =
        loadedCheerio(".cat-series").first().text() || loadedCheerio("h1.leading-none ~ span").first().text() || "";
      if (chapterSubtitle) {
    ...
    if (chapterText) {
       ...
       */
      chapterText = chapterText.replace(/href="\//g, `href="${this.getLocation(result.url)}/`);
    }
  ...
  async searchNovels(searchTerm: string, page: number): Promise<Plugin.NovelItem[]> {
    ...
    const longestSearchTerm = splits.reduce((a, b) => (a.length > b.length ? a : b), "");
