...
class WTRLAB implements Plugin.PluginBase {
  ...
  async popularNovels(
    ...
    if (showLatestNovels) {
      ...
      const novels: Plugin.NovelItem[] = loadedCheerio(".serie-item")
        .map((index, element) => ({
          name:
            loadedCheerio(element).find(".title-wrap > a").text().replace(loadedCheerio(element).find(".rawtitle").text(), "") ||
            "",
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    const chapters: Plugin.ChapterItem[] = jsonData.props.pageProps.serie.chapters.map((jsonChapter, chapterIndex) => ({
      ...
      releaseTime: (jsonChapter?.created_at || jsonChapter?.updated_at)?.substring(0, 10),
