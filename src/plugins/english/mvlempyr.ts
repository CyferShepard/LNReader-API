...
class MVLEMPYRPlugin implements Plugin.PluginBase {
  ...
  checkCaptcha(loadedCheerio: CheerioAPI) {
    ...
    if (title === "Attention Required! | Cloudflare" || title === "Just a moment...")
      throw new Error("Captcha error, please open in webview");
  }

  parseNovels(loadedCheerio: CheerioAPI, nextPageConsumer?: (nextPage: string, pageCount: number) => void) {
    this.checkCaptcha(loadedCheerio);

    if (nextPageConsumer) {
      const nextPage = loadedCheerio("a.w-pagination-next.next").attr("href");
      if (nextPage) nextPageConsumer(nextPage, parseInt(loadedCheerio("div.w-page-count.hide").text().split(" / ").pop()!));
    }
  ...
  parseNovelsFast(body: string) {
    ...
    for (const novelInfo of novelInfosStart) {
      const realNovelInfo = novelInfo.split("READ</a></div></div></div>")[0] + "READ</a></div></div></div>";
  ...
  parseNovelHtmlNew(el: string): Plugin.NovelItem & ExtraNovelData {
    ...
    if (!parserData.parser) {
      parserData.parser = new Parser({
        onopentag(name: string, attribs: Record<string, string>, isImplied: boolean) {
          ...
          if (attribs["class"]?.includes?.("ratingwrapper")) parserData.depthRatingWrapper = 1;

          if (name === "h2" && attribs["fs-cmsfilter-field"] === "name") parserData.inTag = "name";
          if (name === "a" && !parserData.ret!.path) parserData.ret!.path = attribs["href"].replace(/^\//, "");
          if (name === "img" && !parserData.ret!.cover) parserData.ret!.cover = attribs["src"];
          if (name === "div" && attribs["fs-cmssort-field"] === "avgr" && parserData.depthRatingWrapper)
            parserData.inTag = "avgReview";
          if (name === "div" && attribs["fs-cmssort-field"] === "reviews" && parserData.depthRatingWrapper)
            parserData.inTag = "reviewCount";
          if (name === "div" && attribs["fs-cmssort-field"] === "chapter" && attribs["class"]?.includes?.("chapter-count"))
            parserData.inTag = "chapterCount";
          if (name === "div" && attribs["fs-cmssort-field"] === "update") parserData.inTag = "updated";
          if (name === "div" && attribs["fs-cmssort-field"] === "crdate") parserData.inTag = "created";
          if (name === "div" && attribs["fs-cmsnest-collection"] === "genre") parserData.inTag = "genres";
          if (name === "div" && attribs["fs-cmsnest-collection"] === "tags") parserData.inTag = "tags";
        ...
        ontext(data: string) {
          ...
          // @ts-ignore
          parserData.ret[parserData.inTag] = (parserData.ret[parserData.inTag] || "") + data;
        },
  ...
  parseNovelHtml(el: CheerioAPI) {
    return {
      ...
      avgReview: parseFloat(el('.ratingwrapper > div[fs-cmssort-field="avgr"]').text()),
      reviewCount: parseFloat(el('.ratingwrapper > div[fs-cmssort-field="reviews"]').text()),
      chapterCount: parseFloat(el('div[fs-cmssort-field="chapter"].chapter-count').text()),
  ...
  async popularNovels(pageNo: number, { filters }: Plugin.PopularNovelsOptions<Filters>): Promise<Plugin.NovelItem[]> {
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    const posts = (
      await Promise.all(
        new Array(Math.ceil(tags.count / 500))
          ...
          .map((_, i) => i + 1)
          .map((page) =>
            fetchApi(this._chapSite + "wp-json/wp/v2/posts?tags=" + tags.id + "&per_page=500&page=" + page, {
              headers: {
                origin2: this.site.replace(/\/$/, ""),
                origin: this.site.replace(/\/$/, ""),
              },
    ...
    return {
      path: novelPath,
      name: loadedCheerio("div.image-container.w-embed > img").attr("alt") || "Untitled",
  ...
  async searchNovels(searchTerm: string, page: number): Promise<Plugin.NovelItem[]> {
    ...
    const searchResults = allNovels.filter((novel) => novel.name.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  async loadAll() {
    ...
    let pageCount = 1;
    const pages = [
      ...(await this.parseNovelListPage(`${this.site}advance-search`, (a, b) => {
    ...
    for (let i = 2; i <= pageCount; i++) {
      followingPages.push(this.parseNovelListPage(`${this.site}advance-search${pageQueryId}_page=${i}`));
    }
