...
class DreamBigTL implements Plugin.PluginBase {
  ...
  async popularNovels(pageNo: number, { showLatestNovels }: Plugin.PopularNovelsOptions): Promise<Plugin.NovelItem[]> {
    ...
    loadedCheerio("#webify-pro-main-nav-menu > li.has-sub").each((_, categoryElem) => {
      const categoryName = loadedCheerio(categoryElem).children("a").first().text().trim();
  ...
  async parseChapters(loadedCheerio: CheerioAPI): Promise<Plugin.ChapterItem[]> {
    ...
    if (chapters.length === 0) {
      loadedCheerio('h2:contains("List of Chapters"), span:contains("List of Chapters")').each((_, header) => {
  ...
  async searchNovels(searchTerm: string): Promise<Plugin.NovelItem[]> {
    ...
    loadedCheerio(".blog-posts.index-post-wrap .blog-post.hentry.index-post").each((_, ele) => {
      ...
      const novelCover = loadedCheerio(ele).find(".entry-image").attr("data-image") || "";
