...
class NovelFire implements Plugin.PluginBase {
  ...
  async popularNovels(
    ...
    return loadedCheerio(".novel-item")
      .map((index, ele) => {
        const novelName = loadedCheerio(ele).find(".novel-title > a").attr("title") || "No Title Found";
        const novelCover = loadedCheerio(ele).find(".novel-cover > img").attr("data-src");
  ...
  async parseChapters(novelPath: string, pages: number): Promise<Plugin.ChapterItem[]> {
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    const novel: Plugin.SourceNovel = {
      ...
      author: loadedCheerio(".author .property-item > span").text() || "No Author Found",
      status:
        loadedCheerio(".header-stats .ongoing").text() || loadedCheerio(".header-stats .completed").text() || "No Status Found",
    };

    const totalChapters = loadedCheerio(".header-stats .icon-book-open").parent().text().trim();
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    ...
    bloatElements.forEach((tag) => {
      if (tag instanceof RegExp) {
        loadedCheerio("*")
          .filter((_, el) => tag.test(loadedCheerio(el).prop("tagName")!.toLowerCase()))
          .remove();
      } else {
  ...
  async searchNovels(searchTerm: string, page: number): Promise<Plugin.NovelItem[]> {
    ...
    return loadedCheerio(".novel-list.chapters .novel-item")
      .map((index, ele) => {
        ...
        const novelCover = loadedCheerio(ele).find(".novel-cover > img").attr("src");
