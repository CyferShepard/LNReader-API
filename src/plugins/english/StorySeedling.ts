...
class StorySeedlingPlugin implements Plugin.PluginBase {
  ...
  async popularNovels(pageNo: number): Promise<Plugin.NovelItem[]> {
    ...
    const postValue = loadedCheerio("div[ax-load][x-data]").attr("x-data")?.replace("browse('", "").replace("')", "") as string;
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    loadedCheerio("div.grid.w-full.grid-cols-1.gap-4.md\\:grid-cols-2 > a").each(function () {
      ...
      const releaseTime = loadedCheerio(this).find("div > div > small").text().trim();
