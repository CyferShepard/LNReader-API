...
class NovelBuddy implements Plugin.PluginBase {
  ...
  parseNovels(loadedCheerio: CheerioAPI) {
    ...
    loadedCheerio(".book-item").each((idx, ele) => {
      ...
      const novelCover = "https:" + loadedCheerio(ele).find("img").attr("data-src");
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    const getChapters = async (id: string) => {
      ...
      loadedCheerio("li").each((i, el) => {
        const chapterName = loadedCheerio(el).find(".chapter-title").text().trim();

        const releaseDate = loadedCheerio(el).find(".chapter-update").text().trim();

        const months = ["jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec"];
        const monthsJoined = months.join("|");

        const rx = new RegExp(`(${monthsJoined}) (\\d{1,2}), (\\d{4})`, "i").exec(releaseDate);
        if (!rx) return;
        const year = +rx[3];
