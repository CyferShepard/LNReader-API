...
class VyNovel implements Plugin.PluginBase {
  ...
  async fetchNovels(
    ...
    loadedCheerio('div[class="comic-item"] > a').each((index, element) => {
      const name = loadedCheerio(element).find('div[class="comic-title"]').text()?.trim();
      const cover = loadedCheerio(element).find('div[class="comic-image lozad "]').attr("data-background-image") || defaultCover;
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    const novel: Plugin.SourceNovel = {
      ...
      cover: loadedCheerio('div[class="img-manga"] > img').attr("src") || defaultCover,
      ...
      status: loadedCheerio('span[class="text-ongoing"]').text() === "Ongoing" ? NovelStatus.Ongoing : NovelStatus.Completed,
    };

    const chapters: Plugin.ChapterItem[] = [];
    const totalChapters = loadedCheerio('div[class="list-group"] > a').length;

    loadedCheerio('div[class="list-group"] > a').each((chapterIndex, element) => {
  ...
  resolveUrl = (path: string, isNovel?: boolean) => this.site + (isNovel ? "/novel/" : "/read/") + path;
