...
class LibReadPlugin implements Plugin.PluginBase {
  ...
  async getCheerio(url: string): Promise<CheerioAPI> {
    ...
    if (!r.ok) throw new Error(`Could not reach site (${r.status}: ${r.statusText}) try to open in webview.`);
  ...
  async popularNovels(
    ...
    if (showLatestNovels) url += "/sort/latest-novels/";
      ...
      if (filters && filters.type_genre && filters.type_genre.value !== "all" && filters.type_genre.value !== "genre")
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    const chapters: Plugin.ChapterItem[] = loadedCheerio("#idData > li > a")
      .map((chapterIndex, element) => ({
        ...
        path: loadedCheerio(element).attr("href") || novelPath.replace(".html", "/chapter-" + (chapterIndex + 1) + ".html"),
        releaseTime: null,
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    ...
    if (loadedCheerio("style").text().includes("p:nth-last-child(1)")) loadedCheerio("div.txt").find("p:last-child").remove();
