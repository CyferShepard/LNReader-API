...
class Webnovel implements Plugin.PluginBase {
  ...
  async parseNovels(loadedCheerio: CheerioAPI, category_bool: boolean, search_bool: boolean): Promise<Plugin.NovelItem[]> {
    const selector = category_bool ? ".j_category_wrapper" : search_bool ? ".j_list_container" : "";
    const attribute = category_bool ? "data-original" : search_bool ? "src" : "";

    return loadedCheerio(`${selector} li`)
      .map((i_, ele) => {
        const novelName = loadedCheerio(ele).find(".g_thumb").attr("title") || "No Title Found";
        const novelCover = loadedCheerio(ele).find(".g_thumb > img").attr(attribute);
  ...
  async parseChapters(novelPath: string): Promise<Plugin.ChapterItem[]> {
    ...
    loadedCheerio(".volume-item").each((i_v_, ele_v) => {
      ...
      let volumeName = volumeNameMatch ? `Volume ${volumeNameMatch[1]}` : "Unknown Volume";

      loadedCheerio(ele_v)
        .find("li")
        .each((i_c_, ele_c) => {
          const chapterName = `${volumeName}: ` + (loadedCheerio(ele_c).find("a").attr("title")?.trim() || "No Title Found");
          const chapterPath = loadedCheerio(ele_c).find("a").attr("href");

          if (chapterPath) {
            chapters.push({
              name: loadedCheerio(ele_c).find("svg").length ? `${chapterName} 🔒` : chapterName,
              path: chapterPath,
            });
          }
        });
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    ...
    return loadedCheerio(".cha-tit").html()! + loadedCheerio(".cha-words").html()!;
  }
