...
class SakuraNovel implements Plugin.PluginBase {
  ...
  parseNovels(loadedCheerio: CheerioAPI) {
    ...
    loadedCheerio(".flexbox2-item").each((i, el) => {
      const novelName = loadedCheerio(el).find(".flexbox2-title span").first().text();
      const novelCover = loadedCheerio(el).find("img").attr("src");
      const novelUrl = loadedCheerio(el).find(".flexbox2-content > a").attr("href");
  ...
  async popularNovels(page: number, { filters }: Plugin.PopularNovelsOptions<typeof this.filters>): Promise<Plugin.NovelItem[]> {
    ...
    if (filters.lang.value.length) link += filters.lang.value.map((i) => `&country[]=${i}`).join("");
    if (filters.genre.value.length) link += filters.genre.value.map((i) => `&genre[]=${i}`).join("");
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    ...
    const divi = loadedCheerio("div:contains('Daftar Isi') +").find("div:first").attr("class");
