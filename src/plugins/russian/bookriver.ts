...
class Bookriver implements Plugin.PluginBase {
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    const result = await fetchApi(this.resolveUrl(novelPath, true)).then((res) => res.text());
    ...
    const jsonRaw = result.match(regex);
    if (jsonRaw instanceof Array && jsonRaw[1]) {
      const book: BooksEntity = JSON.parse(jsonRaw[1]).props.pageProps.state.book.bookPage;
      ...
      novel.status = book?.statusComplete === "writing" ? NovelStatus.Ongoing : NovelStatus.Completed;

      if (book.tags?.length) novel.genres = book.tags?.map((tag) => tag.name).join(",");
        ...
        book.ebook.chapters.forEach((chapter, chapterIndex) => {
          if (chapter.available) {
            chapters.push({
              ...
              releaseTime: dayjs(chapter.firstPublishedAt || chapter.createdAt || undefined).format("LLL"),
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    ...
    const { data }: responseChapter = await fetchApi(url + chapterPath.split("/").pop()).then((res) => res.json());
  ...
  async searchNovels(searchTerm: string, pageNo: number | undefined = 1): Promise<Plugin.NovelItem[]> {
    const url = this.apiSite + "search/autocomplete?keyword=" + searchTerm + "&page=" + pageNo + "&perPage=10";
