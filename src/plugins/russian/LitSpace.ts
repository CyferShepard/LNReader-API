...
class freedlit implements Plugin.PluginBase {
  ...
  async popularNovels(
    ...
    if (filters?.genre?.value?.include?.length) {
      url += filters.genre.value.include.map((id) => "&genres_included[]=" + id).join("");
    }

    if (filters?.genre?.value?.exclude?.length) {
      url += filters.genre.value.exclude.map((id) => "&genres_excluded[]=" + id).join("");
    }
    ...
    loadedCheerio("#bookListBlock > div > div").each((index, element) => {
      ...
      const cover = loadedCheerio(element).find("div > a > img").attr("src")?.trim();
