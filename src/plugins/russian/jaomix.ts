...
class Jaomix implements Plugin.PluginBase {
  ...
  async popularNovels(
    ...
    if (filters?.lang?.value?.length) {
      url += filters.lang.value.map((lang, idx) => `&lang[${idx}]=${lang}`).join("");
    }

    if (filters?.genre?.value?.include?.length) {
      url += filters.genre.value.include.map((genre, idx) => `&genre[${idx}]=${genre}`).join("");
    }

    if (filters?.genre?.value?.exclude?.length) {
      url += filters.genre.value.exclude.map((genre, idx) => `&delgenre[${idx}]=del ${genre}`).join("");
    }
    ...
    loadedCheerio('div[class="block-home"] > div[class="one"]').each((index, element) => {
      const name = loadedCheerio(element).find('div[class="img-home"] > a').attr("title");
      const cover = loadedCheerio(element).find('div[class="img-home"] > a > img').attr("src")?.replace("-150x150", "");
      const url = loadedCheerio(element).find('div[class="img-home"] > a').attr("href");
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    loadedCheerio("#info-book > p").each(function () {
      ...
      if (text[0] === "Автор:") {
        ...
        novel.status = text.includes("продолжается") ? NovelStatus.Ongoing : NovelStatus.Completed;
      }
    });
