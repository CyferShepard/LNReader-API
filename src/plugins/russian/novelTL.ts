...
class TL implements Plugin.PluginBase {
  ...
  async fetchNovels(
    ...
    data?.projects?.content?.forEach((novel) =>
      novels.push({
        name: novel.title,
        path: novel.fullUrl,
        cover: novel?.covers?.[0]?.url ? this.site + novel.covers[0].url : defaultCover,
      })
    );
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    const novel: Plugin.SourceNovel = {
      ...
      cover: data.project?.covers?.[0]?.url ? this.site + data.project.covers[0].url : defaultCover,
      summary: data.project?.annotation?.text,
      status: statusKey[data.project?.translationStatus || "unknown"] || NovelStatus.Unknown,
    };
    ...
    data.project?.persons?.forEach((person) => {
      if (person.role == "author" && person.name.firstName) {
        novel.author = person.name.firstName + " " + (person.name?.lastName || "");
      }
      if (person.role == "illustrator" && person.name.firstName) {
        novel.artist = person.name.firstName + " " + (person.name?.lastName || "");
      }
    });
