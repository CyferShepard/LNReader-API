...
class RNBH implements Plugin.PluginBase {
  ...
  async popularNovels(
    ...
    url += showLatestNovels ? "last_chapter_at" : filters?.sort?.value || "computed_rating";
    ...
    if (filters) {
      ...
      const includeTags = [filters.tags?.value?.include, filters.events?.value?.include].flat().filter((t) => t);

      if (includeTags.length) {
        url += "&tags:positive=" + includeTags.join(",");
      }

      const excludeTags = [filters.tags?.value?.exclude, filters.events?.value?.exclude].flat().filter((t) => t);

      if (excludeTags.length) {
        url += "&tags:negative=" + excludeTags.join(",");
      }
    }
    const { resource }: { resource: responseNovels[] } = await fetchApi(url + "&take=40").then((res) => res.json());
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    const { data }: { data: responseNovel } = await fetchApi(this.site + "/api/ranobe/" + novelPath).then((res) => res.json());
    ...
    const chaptersJSON: { volumes: VolumesEntity[] } = await fetchApi(this.site + "/api/ranobe/" + novelPath + "/contents").then(
      (res) => res.json()
    );
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    ...
    const chapterText = body.substring(indexA, indexB).replace(/<img data-media-id="(.*?)".*?>/g, '<img src="/api/media/$1">');
  ...
  async searchNovels(searchTerm: string): Promise<Plugin.NovelItem[]> {
    ...
    result
      ...
      ?.data?.forEach((novel) =>
        novels.push({
          name: novel?.names?.rus || novel?.names?.eng || novel.name || novel?.names?.original || "",
          path: novel.id.toString(),
