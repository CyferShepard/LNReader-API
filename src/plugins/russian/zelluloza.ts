...
class Zelluloza implements Plugin.PluginBase {
  ...
  async popularNovels(
    ...
    loadedCheerio('section[class="book-card-item"]').each((index, element) => {
      novels.push({
        ...
        cover: this.site + loadedCheerio(element).find('img[class="shadow"]').attr("src"),
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    const novel: Plugin.SourceNovel = {
      ...
      summary: loadedCheerio("#bann_full").text() || loadedCheerio("#bann_short").text(),
      author: loadedCheerio(".author_link").text(),
      status: loadedCheerio(".tech_decription").text().includes("Пишется") ? NovelStatus.Ongoing : NovelStatus.Completed,
    };

    const chapters: Plugin.ChapterItem[] = [];
    loadedCheerio('ul[class="g0"] div[class="w800_m"]').each((chapterIndex, element) => {
      ...
      if (isFree) {
        ...
        const releaseDate = loadedCheerio(element).find('div[class="stat"]').text();
        chapters.push({
  ...
  parseDate = (dateString: string | undefined = "") => {
    const [, day, month, year, hour, minute] = dateString.match(/(\d{2})\.(\d{2})\.(\d{4})г (\d{2}):(\d{2})/) || [];

    if (day && month && year && hour && minute) {
      return dayjs(year + "-" + month + "-" + day + " " + hour + ":" + minute).format("LLL");
    }
