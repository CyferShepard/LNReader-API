...
class ReN implements Plugin.PluginBase {
  ...
  async popularNovels(
    ...
    const { content }: { content: responseNovels[] } = await fetchApi(url).then((res) => res.json());
  ...
  async parseNovel(novelID: string): Promise<Plugin.SourceNovel> {
    const { content }: { content: responseNovel } = await fetchApi(this.site + "/api/titles/" + novelID).then((res) =>
      res.json()
    );
    ...
    const novel: Plugin.SourceNovel = {
      ...
      cover: this.site + (content.img.high || content.img.mid || content.img.low),
      status: content.status.name === "Продолжается" ? NovelStatus.Ongoing : NovelStatus.Completed,
    };
    ...
    const totalPages = (content.branches?.[0]?.count_chapters || content.count_chapters) / 100;
    ...
    for (let page = 0; page < totalPages; page++) {
      ...
      } = await fetchApi(this.site + "/api/titles/chapters/?branch_id=" + branch_id + "&count=100&page=" + (page + 1)).then(
        (res) => res.json()
      );
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    ...
    const result: { content: responseСhapter } = await fetchApi(url).then((res) => res.json());
