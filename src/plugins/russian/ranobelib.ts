...
import { storage, localStorage } from "@libs/storage.ts";
...
class RLIB implements Plugin.PluginBase {
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    if (data.teams.length) {
      data.teams.forEach(({ name, details }) => (branch_name[details?.branch_id || "0"] = name));
    }

    const chaptersJSON: { data: DataChapter[] } = await fetchApi(this.apiSite + novelPath + "/chapters", {
      headers: this.user?.token,
    }).then((res) => res.json());
    ...
    if (chaptersJSON.data.length) {
      ...
      chaptersJSON.data.forEach((chapter) =>
        chapter.branches.forEach(({ branch_id, created_at }) =>
          chapters.push({
            name: "Том " + chapter.volume + " Глава " + chapter.number + (chapter.name ? " " + chapter.name : ""),
            path: novelPath + "/" + chapter.volume + "/" + chapter.number + "/" + (branch_id || ""),
  ...
  resolveUrl = (path: string, isNovel?: boolean) => {
    ...
    const chapterPath = slug + "/read/v" + volume + "/c" + number + (branch_id ? "?bid=" + branch_id : "");

    return this.site + "/ru/" + chapterPath + (ui ? (branch_id ? "&" : "?") + ui : "");
  };
      ...
      case "image":
        if (element.attrs?.images?.length) {
          element.attrs.images.forEach(({ image }) => {
            const file = images.find((file) => file.name == image || file.id == image);
            if (file) {
