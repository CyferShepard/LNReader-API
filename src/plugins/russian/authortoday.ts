...
class AuthorToday implements Plugin.PluginBase {
  ...
  async popularNovels(pageNo: number, { showLatestNovels, filters }: Plugin.PopularNovelsOptions): Promise<Plugin.NovelItem[]> {
    ...
    url += "&sorting=" + (showLatestNovels ? "recent" : filters?.sort?.value || "popular");
  ...
  async parseNovel(workID: string): Promise<Plugin.SourceNovel> {
    ...
    const book: responseBook = await fetchApi(this.apiSite + "work/" + workID + "/details", {
      headers: {
    ...
    const novel: Plugin.SourceNovel = {
      ...
      summary: "",
      author: book.originalAuthor || book.authorFIO || book.coAuthorFIO || book.secondCoAuthorFIO || book.translator || "",
    ...
    const chaptersJSON: ChaptersEntity[] = await fetchApi(this.apiSite + "work/" + workID + "/content", {
      headers: {
        Authorization: "Bearer " + this.user?.token || "guest",
      },
    }).then((res) => res.json());
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    ...
    if (!this.user) this.user = await this.getUser();
    const result: encryptedСhapter = await fetchApi(this.apiSite + `work/${workID}/chapter/${chapterID}/text`, {
      headers: {
        Authorization: "Bearer " + this.user?.token || "guest",
      },
    }).then((res) => res.json());
  ...
  async searchNovels(searchTerm: string, pageNo: number | undefined = 1): Promise<Plugin.NovelItem[]> {
    const url = this.site + "/search?category=works&q=" + searchTerm + "&page=" + pageNo;
    const result = await fetchApi(url).then((res) => res.text());
    const loadedCheerio = parseHTML(result);
    const novels: Plugin.NovelItem[] = [];

    loadedCheerio("a.work-row").each((index, element) => {
      const name = loadedCheerio(element).find('h4[class="work-title"]').text().trim();
  ...
  resolveUrl = (path: string, isNovel?: boolean) => this.site + (isNovel ? "/work/" : "/reader/") + path;

  user: authorization | undefined;
  getUser = async () => {
    let user = storage.get("user") || { userId: "", token: "guest" };
    if (user && user.userId && user.token) {
      const currentUser: currentUser = await fetchApi(this.apiSite + "account/current-user", {
        headers: {
...
function decrypt(encrypt: string, encryptedKey: string, userId: number | string = "") {
