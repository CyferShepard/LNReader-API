...
import { HTMLParser2Util, Plugin } from "@typings/plugin.ts";
...
class HakoPlugin implements Plugin.PluginBase {
  ...
  popularNovels(pageNo: number, { filters }: Plugin.PopularNovelsOptions<typeof this.filters>): Promise<Plugin.NovelItem[]> {
        ...
        const parser = new Parser({
          onopentag(name, attributes) {
            ...
            if (parseNovelRouter.action) {
              parseNovelRouter.handlers[parseNovelRouter.action]?.onopentag?.(name, attributes);
            }
          },
          ontext(data) {
            if (parseNovelRouter.action) {
              parseNovelRouter.handlers[parseNovelRouter.action]?.ontext?.(data);
            }
          },
          onclosetag(name, isImplied) {
            if (parseNovelRouter.action) {
              parseNovelRouter.handlers[parseNovelRouter.action]?.onclosetag?.(name, isImplied);
            }
  ...
  parseChapter(chapterPath: string): Promise<string> {
    return fetchApi(this.site + chapterPath)
      ...
      .then(
        (html) =>
          html.match(/(<div id="chapter-content".+?>[^]+)<div style="text-align: center;/)?.[1] || "Không tìm thấy nội dung"
      );
  }
