...
class TruyenConect implements Plugin.PagePlugin {
  ...
  async parsePage(novelPath: string, page: string): Promise<Plugin.SourcePage> {
    ...
    const query = `dataEpisodes=${volume.value}&datastory=${volume.story}&dataNavigation=${encodeURIComponent(
      volume.navigation
    )}`;
    const chaptersUrl = `${this.site}/truyen/getreadingchapAction?${query}`;
    await this.sleep(1000);
    const res: { err: number; html: string } = await fetchApi(chaptersUrl).then((r) => r.json());
