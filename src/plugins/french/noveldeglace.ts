...
class NovelDeGlacePlugin implements Plugin.PluginBase {
  ...
  parseDate(date: string): string {
    ...
    return dayjs(`${day} ${monthMapping[month.normalize("NFD").replace(/[\u0300-\u036f]/g, "")]} ${year}`, "D MMMM YYYY").format(
      "DD MMMM YYYY"
    );
  }
  ...
  parseNovels($: CheerioAPI, showLatestNovels: boolean | undefined): Plugin.NovelItem[] {
    ...
    $("article").each((i, el) => {
      ...
      if (showLatestNovels) novelUrl = cheerio.find("span.Roman > a").attr("href");
  ...
  async popularNovels(
    ...
    if (showLatestNovels) url += "chapitre";
      ...
      let cat_gen = "all";
      if (filters && typeof filters.categorie_genre.value == "string") cat_gen = filters.categorie_genre.value;
      ...
      } else if (pageNo > 1) return []; // when asking for all novels, there is only 1 page
      else url += "roman";
    }
    url += "/page/" + pageNo;
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    novel.name = ($("div.entry-content > div > strong")[0].nextSibling as string | null)?.nodeValue?.trim() || "Untitled";
    ...
    novel.author = $("strong:contains('Auteur :')").parent().text().replace("Auteur : ", "").trim();

    novel.artist = $("strong:contains('Illustrateur :')").parent().text().replace("Illustrateur :", "").trim();

    const categorie = $(".categorie").text().replace("Catégorie :", "").trim();
    const genres = $(".genre").text().replace("Genre :", "").replace(/, /g, ",").trim();
    if (categorie && categorie != "Autre") novel.genres = categorie;
    if (genres) novel.genres = novel.genres ? novel.genres + "," + genres : genres;
    ...
    volumes.each((volumeIndex: number, el) => {
      ...
      $(el)
        ...
        .each((chapterIndex: number, el) => {
          const cheerio = $(el);
          const newChapterName = chapterName + cheerio.find("a").first().text().trim() || "";
          if (!cheerio.find("i.fa").length) {
            ...
            const items = cheerio.find("i").parent().next().html()?.split("</a>") || [];
            items?.shift();
            const dates: string[] = [];
            items?.forEach((item) => {
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    ...
    const chapterText = $(".chapter-content").html() || $(".entry-content").html() || "";
    return chapterText;
  }
