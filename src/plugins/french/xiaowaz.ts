import { Cheerio, CheerioAPI, load, Element } from "npm:cheerio";
...
class XiaowazPlugin implements Plugin.PluginBase {
  ...
  async getNovelsCovers(novels: Plugin.NovelItem[]): Promise<Plugin.NovelItem[]> {
    await Promise.all(
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    novel.cover =
      $('img[fetchpriority = "high"]').first().attr("src") || $("img.aligncenter").first().attr("src") || defaultCover;
    ...
    $(".entry-content > p").each((index, element) => {
      ...
      if (balise.find('a[href*="xiaowaz.fr/articles"]').length !== 0) return false;

      const textbalise = balise.text();
      if (PARAGRAPH_EXCLUDE_LIST.some((keyword) => textbalise.includes(keyword))) return false;
  ...
  getGenres(text: string) {
    ...
    let genre = "";
    if (match !== null) {
      genre = match[1].replace("\u00A0", " ").replace("s :", "").replace(":", "").trim();
