...
class ChireadsPlugin implements Plugin.PluginBase {
  ...
  async popularNovels(pageNo: number, { filters, showLatestNovels }: Plugin.PopularNovelsOptions): Promise<Plugin.NovelItem[]> {
    ...
    if (showLatestNovels) url += "/category/translatedtales/page/" + pageNo;
      ...
      if (filters && typeof filters.tag.value === "string" && filters.tag.value !== "all") tag = filters.tag.value;
    ...
    if (showLatestNovels || tag !== "all") {
      ...
      for (let i = 0; i < loop; i++) {
        if (i === 1) $ = await this.getCheerio(this.site + "/category/original/page/" + pageNo);
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    const infos =
      $("div.inform-product-txt > div.inform-intr-col").text().trim() || $("div.inform-inform-data > h6").text().trim();
