...
class Linovelib_tw implements Plugin.PluginBase {
  ...
  async popularNovels(
    ...
    loadedCheerio(".module-rank-booklist .book-layout").each((i, el) => {
      ...
      const novelCover = loadedCheerio(el).find("div.book-cover > img").attr("data-src");
      if (!url) return;
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    novel.status = meta.includes("完結") ? NovelStatus.Completed : NovelStatus.Ongoing;
    ...
    chaptersLoadedCheerio("#volumes .chapter-li:not(.volume-cover)").each((i, el) => {
      if (chaptersLoadedCheerio(el).hasClass("chapter-bar")) {
        ...
        const urlPart = chaptersLoadedCheerio(el).find(".chapter-li-a").attr("href");
      ...
      const chapterName = volumeName + " — " + chaptersLoadedCheerio(el).find(".chapter-index").text().trim();
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    ...
    const addPage = async (pageCheerio: CheerioAPI) => {
      const formatPage = async () => {
        ...
        pageCheerio("#acontentl img.imagecontent").each((i, el) => {
          ...
          const imgSrc = pageCheerio(el).attr("data-src") || pageCheerio(el).attr("src");
          if (imgSrc) {
    ...
    const loadPage = async (url: string) => {
      ...
      pageHasNextPage =
        pageCheerio("#footlink a:last").text() === "下一页" || pageCheerio("#footlink a:last").text() === "下一頁" ? true : false;
  ...
  async searchNovels(searchTerm: string, pageNo: number): Promise<Plugin.NovelItem[]> {
    ...
    pageCheerio(".book-ol .book-layout").each((i, el) => {
      ...
      const novelCover = pageCheerio(el).find("div.book-cover > img").attr("data-src");
