...
class Agitoon implements Plugin.PluginBase {
  ...
  async popularNovels(pageNo: number, { showLatestNovels }: Plugin.PopularNovelsOptions): Promise<Plugin.NovelItem[]> {
    ...
    if (!res.ok) {
      throw new Error(`Failed to get popular novels: (${res.status}: ${res.statusText}) (try to open in webview)`);
    }
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    await this.checkUrl();
    const result = await fetchApi(this.resolveUrl(chapterPath)).then((res) => res.text());
  ...
  resolveUrl(path: string, isNovel?: boolean) {
    if (!Agitoon.url) fetchApi(this.site).then((res) => (Agitoon.url = res.url.replace(/\/$/, "")));
    return (Agitoon.url ? Agitoon.url : this.site) + (isNovel ? "/novel/list/" : "/novel/view/") + path;
  }
}
