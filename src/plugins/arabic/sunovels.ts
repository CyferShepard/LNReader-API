...
class Sunovels implements Plugin.PagePlugin {
    ...
    loadedCheerio(".list-item").each((idx, ele) => {
      loadedCheerio(ele)
        ...
        .each((idx, ele) => {
          ...
          const novelUrl = loadedCheerio(ele).attr("href")?.trim().replace(/^\/*/, "") || "";
  ...
  async parseNovel(novelUrl: string): Promise<Plugin.SourceNovel & { totalPages: number }> {
    ...
    const mainGenres = Array.from(loadedCheerio("div.categories li.tag"))
      ...
      .join(",");
    const statusGenre = Array.from(loadedCheerio("div.header-stats span").eq(3).find("strong"))
      .map((el) => loadedCheerio(el).text().trim())
      .filter((text) => statusWords.has(text));
    novel.genres = `${statusGenre},${mainGenres}`;
    const statusText = Array.from(loadedCheerio("div.header-stats span").eq(3).find("strong"))
      .map((el) => loadedCheerio(el).text().trim())
      .filter((text) => statusWords.has(text))
      .join();
    novel.status =
    ...
    const chapterNumberStr = loadedCheerio("div.header-stats span").first().text().replace(/[^\d]/g, "");
  ...
  async parsePage(novelPath: string, page: string): Promise<Plugin.SourcePage> {
    ...
    loadedCheerio("ul.chaptersList a").each((i, el) => {
      ...
      const dateAttr = loadedCheerio(el).find("time.chapter-update").attr("datetime");
