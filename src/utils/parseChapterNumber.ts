import { isNil } from "npm:lodash-es";

export const parseChapterNumber = (novelName: string, chapterName: string, chapterNumber?: number): number => {
  ...
  const unwanted = new RegExp(/\b(?:v|ver|vol|version|volume|season|s)[^a-z]?[0-9]+/g);
...
const getChapterNumberFromMatch = (match: RegExpMatchArray | null): number | undefined => {
  if (match) {
