...
import parseProto from "npm:protobufjs";

type FetchInit = {
  headers?: Record<string, string | undefined> | Headers;
  method?: string;
  body?: FormData | string;
  [x: string]: string | Record<string, string | undefined> | undefined | FormData | Headers;
};
 ...
 */
export const fetchText = async function (url: string, init?: FetchInit, encoding?: string): Promise<string> {
  init = await makeInit(init);
  console.log(url, init);
  try {
    const res = await fetch(url, init as RequestInit);
...
export const fetchProto = async function <ReturnType>(protoInit: ProtoRequestInit, url: string, init?: FetchInit) {
  const protoRoot = parseProto.parse(protoInit.proto).root;
